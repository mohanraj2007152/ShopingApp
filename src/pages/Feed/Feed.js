...
class Feed extends Component {
  ...
  componentDidMount() {
    ...
    socket.on('posts', data => {
       ...
       // alert(JSON.stringify('result--->',data.post));
  ...
  addPost = post => {
   // alert(JSON.stringify('result--->',post));
    this.setState(prevState => {
      
      const updatedPosts = [...prevState.posts];
     // alert(prevState.postPage);
  ...
  loadPosts = direction => {
    ...
    fetch('http://localhost:8080/admin/products?page=' + page, {
      headers: {
        Authorization: 'Bearer ' + this.props.token
      }
    })
      ...
      .then(resData => {
        ...
        this.setState({
           ...
           posts: resData.posts.rows.map(post => {
             return {
               ...post,
               imagePath: post.imageUrl
             };
           }),
          totalPosts: resData.posts.count,
          postsLoading: false
        });
      })
      .catch(this.catchError);
  ...
  finishEditHandler = postData => {
    ...
    let url = 'http://localhost:8080/admin/product';
    let method = 'POST';
    if (this.state.editPost) {
      
      url = 'http://localhost:8080/admin/product';
      method = 'PUT';
      formData.append('id', this.state.editPost.id);
    }
    //alert(url);
   // alert(formData.body);
  ...
  deletePostHandler = postId => {
    ...
    //alert(postId);
    fetch('http://localhost:8080/admin/product/' + postId, {
      method: 'DELETE',
      headers: {
        Authorization: 'Bearer ' + this.props.token
      }
